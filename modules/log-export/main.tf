...
resource "google_logging_project_sink" "sink" {
  count                  = local.is_project_level && var.create_sink ? 1 : 0
...
resource "google_logging_folder_sink" "sink" {
  count            = local.is_folder_level && var.create_sink ? 1 : 0
  name             = var.log_sink_name
...
resource "google_logging_organization_sink" "sink" {
  count            = local.is_org_level && var.create_sink ? 1 : 0
  name             = var.log_sink_name
...
resource "google_logging_billing_account_sink" "sink" {
  count           = local.is_billing_level && var.create_sink ? 1 : 0
  name            = var.log_sink_name
...
resource "google_project_iam_member" "bigquery_sink_member" {
  count   = (local.is_billing_level || local.is_org_level || local.is_folder_level || local.is_project_level) && var.create_sink ? 1 : 0
